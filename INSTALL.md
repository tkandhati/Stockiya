...
| `STOCKYA_UNIVERSE` | `nifty300` | `nifty50` (fast) / `nifty100` / `nifty200` / `nifty300` (default; top-300 slice of the curated 500 set — not an official index) / `nifty500` (~456 tickers, curated) / `custom` (reads `config/universe_custom.txt`, one ticker per line — use for exact official constituents). |
