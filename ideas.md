...
> **PARTIALLY SHIPPED 2026-07-26 (see CHANGELOG).** The loader `backend/delivery.py` and an **advisory** delivery-% readout on pick + position cards shipped (drop-zone is `data/delivery/`, MTO format, EQ-only). What remains parked is the part below: feeding delivery into the **composite score** (`delivery_signed_anomaly` in `stages/volume.py`) — that still waits on shadow-trace evidence per gate #1/#2. The advisory display doesn't touch scoring, so the gates below still stand for the scoring change.
